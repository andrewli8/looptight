...
- `_prepare_workers` git-prerequisite guard is covered: `run_swarm` in an empty git repo
  (no commits) reaches line 331 and returns "swarm requires a Git repository with at least
...
- `_changed_files()` in `hook.py` (`hook.py:49`) sets `GIT_TERMINAL_PROMPT=0` in its subprocess env;
  `test_changed_files_sets_git_terminal_prompt_env` in `tests/test_hook.py` now asserts this
...
- `plan_next_tasks`'s `rev-parse HEAD` failure at `swarm.py:644` is covered:
  `test_plan_next_tasks_fails_when_rev_parse_head_fails` in `tests/test_swarm.py`
  monkeypatches `swarm._git` to fail `rev-parse HEAD` after a successful planner commit,
...
- `detect_verify` now checks `pnpm-lock.yaml` → `pnpm test` and `yarn.lock` → `yarn test`
  before `package.json`, symmetric with the `bun.lockb` check. A pnpm or Yarn project may not
...
- The unclosed `/*` arm in `_js_skip_candidate` (discovery.py:448) is mutation-guarded:
  `test_unclosed_block_comment_trailing_skip_is_surfaced` in test_propose.py uses two files —
  one real skip before a trailing `/*`, one skip inside an unclosed `/*` — and asserts exactly
...
- `coordination_scope` docstring corrected: after the Fix-B decision the SQLite coordinator is
  the claim store in every git repo; "file-claims" means the migrate marker is absent, not that
  legacy file claims are in use. The docstring previously said the opposite and would mislead
...
- `detect_verify`'s pnpm-over-yarn priority is locked: `test_detect_verify_pnpm_wins_over_yarn`
  creates both `pnpm-lock.yaml` and `yarn.lock` in the same directory and asserts
...
- `_positive_float` now catches `ValueError` for non-numeric input and re-raises as
  `argparse.ArgumentTypeError`, consistent with the sibling `_non_negative_int` /
...
- `ref_resolves()`'s three early-return guards (`not path_text`, `is_absolute()`, `".." in parts`)
  now have direct unit coverage: `test_ref_resolves_rejects_empty_absolute_and_dotdot_refs` in
  `tests/test_idea_eval.py` passes `""`, `"/etc/passwd"`, and `"../outside.py"` and asserts all
  return `False`; a mutation removing any guard fails the test.
- `_TODO_RE`'s `HACK` and `XXX` alternates now have direct unit coverage:
...
- `_build_prompt` lives in `adapters/base.py` as `build_prompt` and is re-exported
...
- `_FAILURE_LINE_RE`'s `not ok` branch is now mutation-pinned against TAP-format output:
...
- `_session_panel`'s middle early-return sub-condition (`not tasks`) now has
...
- `ClaimStore._read`'s `OSError` arm (`claims.py:146`) now has direct coverage:
  ...
  Commit: `docs(spec): document goal check --json output contract`
  SPEC output contract now names all four `goal check --json` status values
...
- `goal status --json` `done_check`, `continuous`, and `iteration` fields are now
...
- `doctor --json` schema is now documented in `docs/SPEC.md` Output contract section
  (`schema_version`, `command`, `agent`, `verify`, `readiness` with `tier`/`checks`/
...
- `run --limit-max-wait-seconds 0` is rejected at parse time (exit 2); `type=_positive_float`
...
- `_js_comments` now surfaces `// TODO` on the closing line of a multi-line block comment.
  The unconditional `continue` at `discovery.py:259` was replaced: when `in_block` becomes
...
- `test_atomic_write_text_mkdir_oserror_propagates` in `tests/test_fsutil.py` pins the
  `path.parent.mkdir()` failure path at `fsutil.py:22`: a regular file occupying a
  directory-component position causes `NotADirectoryError` (an `OSError` subclass) before any
...
- `pipenv run pytest -q` added to `test_verifier_quality_pdm_run_pytest_is_unit` cases in
  `tests/test_cli.py`: `detect.py:97` auto-detects this command for `Pipfile.lock` projects,
  and the substring "pytest" already caused it to classify as `unit`, but the path was unpinned
  by tests; the new case prevents a refactor from silently breaking the pipenv classification.
...
- `_git_common_dir`'s nonzero-returncode guard (`protocol_commands.py:828`) now has direct
  regression coverage: `test_git_common_dir_returns_none_on_nonzero_returncode` in
  `tests/test_cli.py` monkeypatches `subprocess.run` to return a `CompletedProcess` with
